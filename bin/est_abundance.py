...
def main():
    ...
    # If args.level is something like 'S1', map to the base name 'S' -> 'species'
    if args.level in lvl_dict:
        abundance_lvl = lvl_dict[args.level]
    else:
        # try mapping by first character (handles 'S1', 'G2', etc.)
        if args.level and args.level[0] in lvl_dict:
            abundance_lvl = lvl_dict[args.level[0]]
        else:
            abundance_lvl = args.level
    ...
    while len(curr_nodes) > 0:
        ...
        for child_node in sorted(curr_node.children, key=operator.attrgetter('all_reads')):
            ...
            # FIX: compare first character of level ids (handles args.level like 'S1')
            if child_node.level_id[0] != args.level[0] or child_node.level_id == args.level:
